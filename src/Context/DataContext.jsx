...
const handleState = (state, action) => {
  switch (action.type) {
    case "SET_INITIAL_STATE" : return {...state, masterData : action.payload}
    case "ADD_ITEM": {
      state = {...state, masterData : [...state.masterData, action.payload]}
...
export const DataProvider = ({ children }) => {
  ...
  useEffect(()=>{
    ...
    if (localData) {
      dispatch({type : "SET_INITIAL_STATE", payload : localData.masterData})
    } else {
      dispatch({type : "SET_INITIAL_STATE", payload : inventoryData})
    }
  },[])
  return (
